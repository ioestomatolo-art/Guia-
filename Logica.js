document.addEventListener("DOMContentLoaded", () => {
  ...
  // Catálogo EXACTO: insumos + material + equipo + mobiliario + bienesInformaticos + instrumental
  const catalogo = {
    insumos: [
...
JUEGO.`, stock: "", minimo: "", caducidad: "" },
    ...
    // --- Nueva categoría: instrumental (lista EXACTA proporcionada) ---
    instrumental: [
  ...
  function agregarFila() {
    ...
    // Agregar claves exactas si existen (no inventamos)
    ...
    select.addEventListener("change", () => {
      ...
      const producto = (catalogo[categoriaActiva] || []).find(p => p.clave === clave);
      if (producto) {
        inputDesc.value = producto.descripcion || "";
        if (producto.stock !== undefined) inputStock.value = producto.stock || "";
        if (producto.minimo !== undefined) inputMin.value = producto.minimo || "";
        if (producto.caducidad !== undefined) inputCad.value = producto.caducidad || "";
      } else {
        inputDesc.value = "";
      }
      actualizarFila(tr);
  ...
  // Actualizar fila: estado de stock, días restantes y semáforo por meses
  function actualizarFila(tr) {
    ...
    // Semáforo por meses (rojo <6, amarillo 6-12, verde >12)
  ...
  // Botón enviar: imprime datos a consola
